...
use crate::model::models::{
    ...
    CategoryTemplate, CategoryTemplatePagination, Count, HomeTemplate, NewCategoryTemplate,
    NewPostTemplate, UpdateCategory, UpdateCategoryTemplate,
};
use crate::{
...
pub async fn create_posts_form_ui() -> impl IntoResponse {
    let template = NewPostTemplate { index_sec: &vec![] };
...
pub async fn home_gui() -> impl IntoResponse {
    ...
    let category_list = get_all_categories().await.unwrap();
    let list_iters = category_list.into_iter();
    // for i in list_iters {
    //     psec.push(i.category_name.clone());
    // }
    for category in list_iters {
        psec.push(category.category_name.clone());
    }
    let s = get_connection().await;
    // let number_of_pages: i64;
    let plinks: Vec<String> = Vec::new();
    let pids: Vec<i32> = Vec::new();
    let mut pnav: Vec<i32> = Vec::new();
    let number_of_posts_vector = get_count_of_posts().await.unwrap();
