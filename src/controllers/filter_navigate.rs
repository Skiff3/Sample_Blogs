...
pub async fn blog_pagination(Path((category, page_number)): Path<(String,String)>) -> impl IntoResponse {
    ...
    offset_start = ((page_number_integer-1) * 3);
    ...
    let template = BlogTemplate{index_title: String::from("Blogs"), index_links: &plinks, index_sec: &psec, page_nav_links: &pnav,current_url_page:".".to_string()};
